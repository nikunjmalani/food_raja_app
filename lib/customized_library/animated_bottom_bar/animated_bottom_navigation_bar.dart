...
import 'animatedbottomnavigationbar/circular_notch_and_corner_clipper.dart';
import 'animatedbottomnavigationbar/circular_notched_and_cornered_shape.dart';
import 'animatedbottomnavigationbar/exceptions.dart';
import 'animatedbottomnavigationbar/navigation_bar_item.dart';

class AnimatedBottomNavigationBar extends StatefulWidget {
  /// Icon data to render in the tab bar.
  final List<Map<String, String>> iconWithLabel;
  ...
  AnimatedBottomNavigationBar({
    ...
    this.gapWidth = 72,
    this.fontFamily,
  })  : assert(iconWithLabel != null),
        assert(iconWithLabel.length >= 2 && iconWithLabel.length <= 5),
...
class _AnimatedBottomNavigationBarState
  ...
  List<Widget> _buildItems() {
    List items = <Widget>[];
    for (var i = 0; i < widget.iconWithLabel.length; i++) {
      if (widget.gapLocation == GapLocation.center &&
          i == widget.iconWithLabel.length / 2) {
        items.add(
          GapItem(
      ...
      items.add(
        NavigationBarItem(
          ...
          iconData: widget.iconWithLabel[i],
          iconScale: _iconScale,
          iconSize: widget.iconSize,
          onTap: () => widget
              .onTap(widget.iconWithLabel.indexOf(widget.iconWithLabel[i])),
        ),
      );

      if (widget.gapLocation == GapLocation.end &&
          i == widget.iconWithLabel.length - 1) {
        items.add(
          GapItem(
            width: widget.gapWidth * widget.notchAndCornersAnimation.value,
          ),
